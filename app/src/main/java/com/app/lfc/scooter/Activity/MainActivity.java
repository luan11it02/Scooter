                ...
                database.addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        for (DataSnapshot data: dataSnapshot.getChildren())
                            listUser.add(data.getValue(User.class));
                        for (int i = 0; i < listUser.size(); i++)
                            if (user.getUid().equals(listUser.get(i).getUid()))
                                check = i;
                        if (check != -1){
                            txtNameUser.setText(listUser.get(check).getHoten());
                            txtUser.setText(listUser.get(check).getEmailsdt());
                        } else {
                                    ...
                                    if (edtNameUser.getText().toString().length() > 0) {
                                      ...
                                      /*  SharedPreferences sharedPreferences = getSharedPreferences("NameUser", Context.MODE_PRIVATE);
                                        ...
                                        if (user.getEmail() != null && !user.getEmail().equals(""))
                                            emailsdt = user.getEmail();
                                        else emailsdt = user.getPhoneNumber();
                                        User u = new User(user.getUid(), edtNameUser.getText().toString(), emailsdt);
        ...
        if (currentUser == null){
            ...
            database.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    txtUser.setText(user.getEmailsdt());
                    txtNameUser.setText(user.getHoten());
                }
